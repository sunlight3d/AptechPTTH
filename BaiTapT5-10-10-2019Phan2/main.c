...
struct Item* input(){
    ...
    int i;
    for(i = 0; i < N; i++){
        printf("Enter product's name:[%d]: \n", i+1); scanf("%s", (*(item+i)).name);
        printf("Enter product's rate [%d]: ", i+1); scanf("%d", &((*(item+i)).rate));
        printf("Enter product's quantity: ",); scanf("%d", &((*(item+i)).quantity));
        printf("Enter product's price: "); scanf("%d", &((*(item+i)).price));
    }
    printf("hahaa");
    return item;
}
